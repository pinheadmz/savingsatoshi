...
function verify(sig_r, sig_s, pubkey_x, pubkey_y, msg) {
  ...
  const u1 = (msg * sig_s_inverted) % secp256k1.ORDER;
  const u2 = (sig_r * sig_s_inverted) % secp256k1.ORDER;
  const R = (secp256k1.G.mul(u1)).add(key.mul(u2));
  return R.x.equals(new secp256k1.FE(sig_r));
}
...
function encode_message(text) {
  ...
  const text_bytes = Buffer.from(text, 'ascii');
  const vector = Buffer.concat([
    Buffer.from([prefix.length]),
    prefix,
    Buffer.from([text_bytes.length]),
    text_bytes])
...
function decode_sig(base64_sig) {
  ...
  const vp_sig_bytes = Buffer.from(base64_sig, 'base64');
  const vp_sig_r = BigInt('0x' + vp_sig_bytes.slice(1, 33).toString('hex'));
  const vp_sig_s = BigInt('0x' + vp_sig_bytes.slice(33).toString('hex'));
